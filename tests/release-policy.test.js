...
function runReleasePolicy(t, environment) {
  const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agent-dock-release-policy-'));
...
test('release identity and documentation stay aligned', () => {
  const readme = fs.readFileSync(readmePath, 'utf8');

  assert.match(readme, new RegExp(`当前代码版本：\\*\\*${packageVersion.replaceAll('.', '\\.')}\\*\\*`));
  assert.equal(packageConfig.build.appId, 'com.xiaopu.agentdock');
  assert.equal(packageConfig.build.productName, 'Agent Dock');
  assert.equal(packageConfig.build.artifactName, 'Agent-Dock-${version}-${arch}.${ext}');
});
