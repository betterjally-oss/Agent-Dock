...
const AGENT_DOCK_USER_DATA_PATH = IS_TEST && path.isAbsolute(process.env.AGENT_DOCK_TEST_USER_DATA || '')
  ? process.env.AGENT_DOCK_TEST_USER_DATA
  : path.join(app.getPath('appData'), 'Agent Dock');
app.setName('Agent Dock');
app.setPath('userData', AGENT_DOCK_USER_DATA_PATH);
...
// 窗口置于 screen-saver 层级，因此折叠条仍可接收点击。
...
const AGENT_RUNS_FILE = 'agent-runs.json';
...
const testNotificationPort = Number(process.env.AGENT_DOCK_TEST_PORT);
const TASK_NOTIFICATION_PORT = process.env.NODE_ENV === 'test'
  && Number.isInteger(testNotificationPort)
  && testNotificationPort > 0
  && testNotificationPort < 65536
  ? testNotificationPort
  : 43822;
// /notify/<source> 的来源白名单：只放行已知 Agent，其余一律 404。
const TASK_NOTIFICATION_SOURCES = new Set(['codex', 'gpt', 'claude', 'workbuddy', 'gemini']);
const TODO_REMINDER_LEAD_MS = 60 * 60 * 1000;
const COMPANION_WIDTH = 420;
const COMPANION_HEIGHT = 220;
const COMPANION_CELEBRATE_MS = 4000;
const COMPANION_PEEK_MS = 6400;
const COMPANION_PLAY_FIRST_MS = 20 * 1000;
const COMPANION_PLAY_INTERVAL_MS = 3 * 60 * 1000;
const COMPANION_PENDING_MS = 30 * 1000;
const COMPANION_QUOTES = {
  greeting: [
    '今天也一起稳稳地来吧', '我已经帮你占好位置啦', '先做一件最小的事', '今天也会有好进展',
    '别急，我陪你慢慢来', '新的一天，开工喵', '你负责前进，我负责陪伴', '先从最清楚的一步开始',
    '给自己一点点启动时间', '今天也要记得呼吸', '小猫已就位，随时陪你', '慢一点也没关系',
  ],
  todo: [
    '这一件也稳稳拿下啦', '完成一项，很厉害', '又向前走了一格', '今天的你很靠谱',
    '清单又轻了一点', '做得好，休息一下也可以', '小猫认证：确实完成', '一件一件来，就是这样',
    '进度条又亮了一格', '你把答案变成了现实', '这次也漂亮收尾', '干得好，继续保持节奏',
  ],
  pomodoro: [
    '这一轮专注完成啦', '时间到，起来动一动吧', '你刚才很专注', '小猫宣布：本轮达成',
    '辛苦了，先看看远处', '一个番茄，扎实入账', '专注有回声，你做到了', '歇一会儿，再出发',
    '这段时间没有被辜负', '番茄结束，给自己点个赞', '专注力已安全送达', '又完成一次认真的陪跑',
  ],
  agent: [
    '搭档刚交回一份成果', '这一轮已经稳稳收尾', '新成果到站，辛苦啦', '小猫已替你守住进度',
  ],
};
...
let permissionBlurDeferred = false;
...
let agentRuns = [];
let agentRunningActive = false;
let agentWaitingActive = false;
let todoReminderTimer = null;
let scheduledTodoReminders = [];

let companionWindow = null;
let companionWindowReady = false;
let companionFocusActive = false;
let companionOverride = null;
let companionOverrideTimer = null;
let companionPlayTimer = null;
let pendingCompanionEvent = null;
let companionSystemPaused = false;
let companionSessionDisabled = false;
...
function getCompanionBounds(display) {
  return getCenteredBounds(COMPANION_WIDTH, COMPANION_HEIGHT, display || getWindowDisplay());
}
...
function nextCompanionQuote(kind) {
  const quotes = COMPANION_QUOTES[kind] || COMPANION_QUOTES.todo;
  const settings = readAppSettings();
  const cursor = Math.max(0, Number(settings.companionQuoteCursor[kind]) || 0);
  settings.companionQuoteCursor[kind] = (cursor + 1) % quotes.length;
  saveAppSettings(settings);
  return quotes[cursor % quotes.length];
}

function clearCompanionOverride() {
  if (companionOverrideTimer) clearTimeout(companionOverrideTimer);
  companionOverrideTimer = null;
  companionOverride = null;
}

function clearCompanionActivity() {
  pendingCompanionEvent = null;
  clearCompanionOverride();
}

function createCompanionWindow() {
  if (companionSessionDisabled) return null;
  if (companionWindow && !companionWindow.isDestroyed()) return companionWindow;
  companionWindowReady = false;
  ...
  installLocalWebContentsGuards(companionWindow.webContents);
  const targetWindow = companionWindow;
  targetWindow.setAlwaysOnTop(true, 'screen-saver', 2);
  targetWindow.setVisibleOnAllWorkspaces(true, { visibleOnFullScreen: true });
  targetWindow.setIgnoreMouseEvents(true, { forward: true });
  targetWindow.loadFile(path.join(__dirname, 'renderer', 'companion.html'));
  targetWindow.webContents.once('did-finish-load', () => {
    if (companionWindow !== targetWindow || targetWindow.isDestroyed()) return;
    companionWindowReady = true;
    syncCompanionWindow();
  });
  targetWindow.webContents.once('did-fail-load', (event, errorCode, errorDescription) => {
    companionSessionDisabled = true;
    console.warn('[companion] failed to load; disabled for this session:', errorCode, errorDescription);
    if (!targetWindow.isDestroyed()) targetWindow.destroy();
  });
  targetWindow.webContents.on('render-process-gone', (event, details) => {
    companionSessionDisabled = true;
    console.warn('[companion] renderer disabled for this session:', details.reason);
    if (!targetWindow.isDestroyed()) targetWindow.destroy();
  });
  targetWindow.on('closed', () => {
    if (companionWindow !== targetWindow) return;
...
function syncCompanionWindow(display) {
  const settings = readAppSettings();
  const presentation = companionPresentation({
    ...
    systemPaused: companionSystemPaused,
    notificationActive: Boolean(activeTaskNotification || taskNotificationQueue.length),
    mode: currentMode,
    focusActive: companionFocusActive || agentRunningActive,
    waitingActive: agentWaitingActive,
    override: companionOverride && companionOverride.state,
  });
  if (!presentation.visible) {
    if (companionWindow && !companionWindow.isDestroyed()) companionWindow.hide();
    return;
  }
  const targetWindow = createCompanionWindow();
  if (!targetWindow || targetWindow.isDestroyed()) return;
  const targetDisplay = display || getWindowDisplay();
  targetWindow.setBounds(getCompanionBounds(targetDisplay));
  if (!companionWindowReady) return;
  targetWindow.webContents.send('companion:state', {
    state: presentation.state,
    message: companionOverride ? companionOverride.message : '',
    notchHeight: getCollapsedHeight(targetDisplay),
  });
  targetWindow.showInactive();
}

function showCompanionCelebration(kind) {
  if (readAppSettings().companionEnabled !== true) return false;
  clearCompanionOverride();
  companionOverride = { state: 'celebrate', message: nextCompanionQuote(kind) };
  syncCompanionWindow();
  companionOverrideTimer = setTimeout(() => {
    companionOverrideTimer = null;
    companionOverride = null;
    syncCompanionWindow();
...
function showCompanionPlay() {
  if (
    readAppSettings().companionEnabled !== true
    || companionFocusActive
    || companionOverride
    || (pendingCompanionEvent && pendingCompanionEvent.expiresAt > Date.now())
    || currentMode !== 'collapsed'
    || companionSystemPaused
    || activeTaskNotification
    || taskNotificationQueue.length
    || !mainWindow
    || !mainWindow.isVisible()
  ) return false;
  companionOverride = { state: 'peek', message: '' };
  syncCompanionWindow();
  companionOverrideTimer = setTimeout(() => {
    companionOverrideTimer = null;
    if (companionOverride && companionOverride.state === 'peek') companionOverride = null;
    syncCompanionWindow();
  }, COMPANION_PEEK_MS);
  return true;
}

function scheduleCompanionPlay(delay = COMPANION_PLAY_INTERVAL_MS) {
  if (companionPlayTimer) clearTimeout(companionPlayTimer);
  companionPlayTimer = setTimeout(() => {
    companionPlayTimer = null;
    showCompanionPlay();
    scheduleCompanionPlay();
  }, delay);
}

function queueCompanionCelebration(kind, ttl = COMPANION_PENDING_MS) {
  if (pendingCompanionEvent && pendingCompanionEvent.expiresAt > Date.now()) return;
  pendingCompanionEvent = { kind, expiresAt: Date.now() + ttl };
}

function consumePendingCompanionCelebration() {
  if (!pendingCompanionEvent) return false;
  if (pendingCompanionEvent.expiresAt <= Date.now()) {
    pendingCompanionEvent = null;
    return false;
  }
  if (
  ...
  const { kind } = pendingCompanionEvent;
  pendingCompanionEvent = null;
  return showCompanionCelebration(kind);
}
...
function applyMode(mode, display) {
  ...
  if (mode === 'collapsed') consumePendingCompanionCelebration();
  syncCompanionWindow(display);
}
...
ipcMain.handle('pomodoro:notify', (event, minutes) => {
  ...
  const settings = readAppSettings();
  if (settings.companionEnabled === true && currentMode === 'expanded') {
    return { ok: true, result: 'toast' };
  }
  if (
    ...
    if (activeTaskNotification || taskNotificationQueue.length) {
      queueCompanionCelebration('pomodoro');
      syncCompanionWindow();
      return { ok: true, result: 'companion-queued' };
    }
    showCompanionCelebration('pomodoro');
    return { ok: true, result: 'companion' };
  }
...
function finishTaskNotification(eventId) {
  ...
  setTimeout(() => {
    ...
    if (!activeTaskNotification && taskNotificationQueue.length === 0) {
      consumePendingCompanionCelebration();
      syncCompanionWindow();
    }
  }, 80);
}
...
function startTaskNotificationServer() {
  ...
  const server = http.createServer((request, response) => {
    ...
    const sourceMatch = /^\/(notify|events)\/([a-z0-9-]{1,32})$/i.exec(requestUrl.pathname);
    const route = sourceMatch ? sourceMatch[1].toLowerCase() : '';
    const requestedSource = sourceMatch ? sourceMatch[2].toLowerCase() : '';
    ...
    request.on('end', () => {
      ...
      if (isSubagentNotification(payload)) {
        sendTaskNotificationResponse(response, 202, { ok: true, result: 'ignored' });
        return;
      }
      const normalized = normalizeAgentEvent(payload, source, { legacy: route === 'notify' });
      if (!normalized.ok) {
        sendTaskNotificationResponse(response, 400, normalized);
        return;
      }
      const ingested = ingestAgentEvent(normalized.event);
      sendTaskNotificationResponse(response, ingested.ok ? 202 : 500, ingested);
    });
    request.on('error', () => {
...
function readAppSettings() {
  ...
  delete storedFeatures.links;
  delete storedFeatures.credentials;
  const quoteCursor = stored.companionQuoteCursor && typeof stored.companionQuoteCursor === 'object'
    ? stored.companionQuoteCursor
    : {};
  return {
    features: { ...DEFAULT_FEATURES, ...storedFeatures, home: true },
    shortcut: isValidPanelShortcut(stored.shortcut) ? stored.shortcut : 'Space',
    companionEnabled: stored.companionEnabled !== false,
    companionGreetingDate: typeof stored.companionGreetingDate === 'string'
      ? stored.companionGreetingDate
      : '',
    companionQuoteCursor: {
      greeting: Math.max(0, Number(quoteCursor.greeting) || 0),
      todo: Math.max(0, Number(quoteCursor.todo) || 0),
      pomodoro: Math.max(0, Number(quoteCursor.pomodoro) || 0),
    },
  };
}

function publicAppSettings() {
  const { companionGreetingDate, companionQuoteCursor, ...settings } = readAppSettings();
  return { ...settings, autoLaunch: isAutoLaunchEnabled() };
}
...
const AGENT_SOURCE_CAPABILITIES = {
  codex: { label: 'Codex', states: ['running', 'waiting', 'completed'], note: 'Codex 生命周期 Hooks' },
  claude: { label: 'Claude Code', states: ['running', 'waiting', 'completed', 'failed'], note: '失败需明确事件' },
  gpt: { label: 'GPT', states: [], note: '由发送端声明' },
  workbuddy: { label: 'WorkBuddy', states: ['running', 'waiting', 'completed', 'failed'], note: 'CodeBuddy Code v1.16+ Hooks' },
  gemini: { label: 'Gemini CLI', states: ['running', 'waiting', 'completed'], note: 'Hooks 暂无明确失败终态' },
};

function loadAgentRuns() {
  const payload = readJsonFile(workspacePath(AGENT_RUNS_FILE), { runs: [] });
  return pruneAgentRuns(Array.isArray(payload.runs) ? payload.runs : []);
}

function saveAgentRuns(runs = agentRuns) {
  return writeJsonFile(workspacePath(AGENT_RUNS_FILE), {
    version: 1,
    updatedAt: Date.now(),
    runs,
  });
...
function publicAgentSources() {
  return Object.entries(AGENT_SOURCE_CAPABILITIES).map(([source, capability]) => {
    const seen = agentRuns.filter((run) => run.source === source);
    const observedStates = [...new Set(seen.map((run) => run.state))];
    return {
      source,
      ...capability,
      states: source === 'gpt' ? observedStates : capability.states,
      lastEventAt: Math.max(0, ...seen.map((run) => Number(run.updatedAt) || 0)),
    };
  });
}

function emitAgentRunsChanged(changedRun = null) {
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('agent-runs:changed', {
      ...agentRunPayload({ limit: 50 }),
      changedRun: agentRunView(changedRun),
    });
  }
}

function syncAgentCompanionActivity() {
  agentWaitingActive = agentRuns.some((run) => run.state === 'waiting');
  agentRunningActive = agentRuns.some((run) => run.state === 'running');
  if (agentWaitingActive && companionOverride && companionOverride.state === 'peek') {
    clearCompanionOverride();
  }
  syncCompanionWindow();
}

function agentRunNotification(run) {
  return {
    eventId: `agent-${run.source}-${run.runId}-${run.updatedAt}`,
    source: run.source,
    taskId: run.runId,
    title: run.title,
    project: run.project,
    detail: run.project ? `已完成 · ${run.project}` : '已完成，可以查看了',
    completedAt: run.updatedAt,
  };
}

function ingestAgentEvent(event) {
  const merged = mergeAgentEvent(agentRuns, event);
  if (merged.status === 'invalid') return { ok: false, error: 'invalid_event' };
  if (['duplicate', 'out_of_order'].includes(merged.status)) {
    return { ok: true, result: merged.status, run: merged.run };
  }
  if (!saveAgentRuns(merged.runs)) return { ok: false, error: 'save_failed' };
  agentRuns = merged.runs;
  syncAgentCompanionActivity();
  emitAgentRunsChanged(merged.run);
  if (merged.run.state === 'completed') {
    queueCompanionCelebration('agent');
    enqueueTaskNotification(agentRunNotification(merged.run));
  }
  return { ok: true, result: merged.status, run: merged.run };
}

function findAgentRun(id) {
  return agentRuns.find((run) => `${run.source}:${run.runId}` === String(id || '')) || null;
}

function agentRunView(run, now = Date.now()) {
  return run && listAgentRuns([run], { limit: 1 }, now)[0]
    ? { ...listAgentRuns([run], { limit: 1 }, now)[0], id: `${run.source}:${run.runId}` }
    : null;
}

function agentRunPayload(filter = {}) {
  return {
    items: listAgentRuns(agentRuns, filter).map((run) => ({ ...run, id: `${run.source}:${run.runId}` })),
    sources: publicAgentSources(),
    server: { host: TASK_NOTIFICATION_HOST, port: TASK_NOTIFICATION_PORT, available: notificationServerAvailable },
  };
}

function copyLegacyWorkspaceAssets(sourceRoot, targetRoot) {
...
async function importLegacyWorkspace(currentStorage) {
  ...
  const legacyPayload = readJsonFile(path.join(selected, WORKSPACE_DATA_FILE), {});
  const legacyStorage = legacyPayload.localStorage && typeof legacyPayload.localStorage === 'object'
    ? legacyPayload.localStorage
    : {};
  if (!Object.keys(legacyStorage).length) return { ok: false, error: 'workspace_not_found' };
  const merged = mergeLegacyWorkspaceStorage(currentStorage, legacyStorage);
  const storage = normalizePortableStorage(merged.storage);
  copyLegacyWorkspaceAssets(selected, workspaceRoot());
  const written = writeJsonFile(workspacePath(WORKSPACE_DATA_FILE), {
    version: 1,
    updatedAt: Date.now(),
    localStorage: storage,
  });
  return written
    ? { ok: true, canceled: false, imported: merged.imported, storage }
...
function copyWorkspaceAssets(sourceRoot, targetRoot) {
  ...
  for (const filename of [WORKSPACE_DATA_FILE, AGENT_RUNS_FILE]) {
...
function refreshTrayMenu() {
  ...
  const menu = Menu.buildFromTemplate([
    ...
    },
    {
      ...
      click: (item) => {
        const next = readAppSettings();
        next.companionEnabled = item.checked;
        if (!item.checked) clearCompanionActivity();
        saveAppSettings(next);
        applyAppSettings();
        refreshTrayMenu();
      },
    ...
    { type: 'separator' },
    {
      ...
      click: () => {
        dialog.showMessageBox({
          ...
          message: 'Agent Dock',
          detail:
            `版本 ${app.getVersion()}\n\n常驻 macOS 刘海下的本地 Agent 工作台。工作区数据默认保存在本机；账号密码与 API Key 由 macOS 安全存储加密。\n\nMIT License`,
          buttons: ['好'],
          defaultId: 0,
          cancelId: 0,
          noLink: true,
...
ipcMain.handle('companion:set-focus', (event, active) => {
  if (!mainWindow || event.sender !== mainWindow.webContents || typeof active !== 'boolean') {
    return { ok: false, error: 'invalid' };
  }
  if (active && companionOverride && companionOverride.state === 'peek') {
    clearCompanionOverride();
  }
  companionFocusActive = active;
  syncCompanionWindow();
  return { ok: true };
});

ipcMain.handle('companion:celebrate', (event, kind) => {
  if (!mainWindow || event.sender !== mainWindow.webContents || kind !== 'todo') {
    return { ok: false, error: 'invalid' };
  }
  if (readAppSettings().companionEnabled !== true) return { ok: true, result: 'disabled' };
  if (currentMode === 'expanded' || activeTaskNotification || taskNotificationQueue.length) {
    queueCompanionCelebration('todo');
    syncCompanionWindow();
    return { ok: true, result: 'queued' };
  }
  return { ok: true, result: showCompanionCelebration('todo') ? 'companion' : 'disabled' };
});

ipcMain.handle('companion:interact', (event) => {
  if (!mainWindow || event.sender !== mainWindow.webContents) return { ok: false, error: 'invalid' };
  if (companionWindow && !companionWindow.isDestroyed() && companionWindowReady) {
    companionWindow.webContents.send('companion:interact');
  }
  const settings = readAppSettings();
  const today = localCalendarDay();
  if (settings.companionEnabled === true && settings.companionGreetingDate !== today) {
    settings.companionGreetingDate = today;
    if (!saveAppSettings(settings)) return { ok: false, error: 'save_failed' };
    queueCompanionCelebration('greeting', 5 * 60 * 1000);
  }
  return { ok: true };
});
...
ipcMain.handle('settings:set-companion', (event, enabled) => {
  if (typeof enabled !== 'boolean') return { ok: false, error: 'invalid' };
  const next = readAppSettings();
  next.companionEnabled = enabled;
  if (!enabled) clearCompanionActivity();
  if (!saveAppSettings(next)) return { ok: false, error: 'save_failed' };
  applyAppSettings();
  refreshTrayMenu();
  return { ok: true, settings: publicAppSettings() };
...
ipcMain.handle('media:microphone', async () => {
  ...
  try {
    ...
    if (mediaPermissionRequests === 0 && permissionBlurDeferred) {
      permissionBlurDeferred = false;
    }
  }
});
...
ipcMain.handle('agent:list', (event, filter) => (
  isMainRenderer(event) ? agentRunPayload(filter) : { items: [], sources: [], server: {} }
));

ipcMain.handle('agent:activate-window', async (event, id) => {
  if (!isMainRenderer(event)) return { ok: false, error: 'forbidden' };
  const run = findAgentRun(id);
  if (!run) return { ok: false, error: 'not_found' };
  const result = await scanCurrentWindows();
  const target = (result.items || [])
    .map((item) => ({ item, score: taskWindowMatchScore(run, item) }))
    .filter((candidate) => candidate.score > 0)
    .sort((left, right) => right.score - left.score)[0];
  if (!target) return { ok: false, error: 'window_not_found' };
  try {
    const focused = (await runJxa(WINDOW_FOCUS_JXA, [target.item.pid, target.item.title, target.item.windowIndex])) === 'true';
    return focused ? { ok: true, approximate: target.score === 10 } : { ok: false, error: 'focus_failed' };
  } catch (error) {
    return { ok: false, error: 'focus_failed' };
  }
});

const AGENT_APP_CANDIDATES = {
  codex: ['Codex', 'ChatGPT'],
  claude: ['Claude'],
  gpt: ['ChatGPT'],
...
ipcMain.handle('agent:open-app', async (event, source) => {
  if (!isMainRenderer(event) || process.platform !== 'darwin') return false;
  const candidates = AGENT_APP_CANDIDATES[String(source || '')];
  if (!candidates) return false;
  for (const appName of candidates) {
    const opened = await new Promise((resolve) => {
      execFile('/usr/bin/open', ['-a', appName], { timeout: 4000 }, (error) => resolve(!error));
    });
...
ipcMain.handle('agent:copy-summary', (event, id) => {
  if (!isMainRenderer(event)) return { ok: false, error: 'forbidden' };
  const run = findAgentRun(id);
  if (!run || !['completed', 'failed'].includes(run.state)) return { ok: false, error: 'not_found' };
  clipboard.writeText(String(run.summary || run.title || '').trim());
  return { ok: true };
});

ipcMain.handle('agent:convert-to-todo', (event, payload) => {
  if (!isMainRenderer(event)) return { ok: false, error: 'forbidden' };
  const run = findAgentRun(payload && payload.id);
  const category = String(payload && payload.category || '');
  if (!run || !['completed', 'failed'].includes(run.state)) return { ok: false, error: 'not_found' };
  if (!['P0', 'P1', 'P2', 'P3'].includes(category)) return { ok: false, error: 'invalid_category' };
  const convertedAt = Date.now();
  const next = agentRuns.map((item) => item === run
    ? { ...item, convertedToTodo: { category, convertedAt } }
    : item);
  if (!saveAgentRuns(next)) return { ok: false, error: 'save_failed' };
  agentRuns = next;
  const updated = findAgentRun(payload.id);
  emitAgentRunsChanged(updated);
  return { ok: true, todo: { text: run.title, category }, run: agentRunView(updated) };
});

ipcMain.handle('agent:import-legacy', (event, currentStorage) => {
  if (!isMainRenderer(event)) return { ok: false, error: 'forbidden' };
  const serialized = JSON.stringify(currentStorage || {});
  if (!currentStorage || typeof currentStorage !== 'object' || Array.isArray(currentStorage)
    || Buffer.byteLength(serialized) > 8 * 1024 * 1024) {
    return { ok: false, error: 'invalid_storage' };
  }
  return importLegacyWorkspace(currentStorage);
});

ipcMain.handle('agent:copy-setup', (event, source) => {
  if (!isMainRenderer(event) || !TASK_NOTIFICATION_SOURCES.has(source)) {
    return { ok: false, error: 'invalid_source' };
  }
  const appRoot = app.isPackaged
    ? path.join(process.resourcesPath, 'app')
    : __dirname;
  const codexHookCommand = `node "${path.join(appRoot, 'scripts', 'codex-notify.js')}"`;
  const compatibleHookCommand = (hookSource) => `node "${path.join(appRoot, 'scripts', 'agent-hook-notify.js')}" ${hookSource}`;
  const snippets = {
    codex: JSON.stringify({
      description: 'Agent Dock real-time Codex status',
      hooks: Object.fromEntries(['UserPromptSubmit', 'PermissionRequest', 'PostToolUse', 'Stop'].map((eventName) => [
        eventName,
        [{ hooks: [{ type: 'command', command: codexHookCommand, async: true, timeout: 3 }] }],
      ])),
    }, null, 2),
    claude: JSON.stringify({
      hooks: Object.fromEntries(['UserPromptSubmit', 'Notification', 'Stop', 'StopFailure'].map((eventName) => [
        eventName,
        [{ hooks: [{ type: 'command', command: `node "${path.join(appRoot, 'scripts', 'claude-notify.js')}"` }] }],
      ])),
    }, null, 2),
    gpt: `curl -X POST http://${TASK_NOTIFICATION_HOST}:${TASK_NOTIFICATION_PORT}/events/gpt -H 'Content-Type: application/json' -d '{"version":1,"event":"completed","run_id":"demo","title":"任务完成","summary":"摘要","occurred_at":${Date.now()}}'`,
    workbuddy: JSON.stringify({
      hooks: Object.fromEntries(['UserPromptSubmit', 'PermissionRequest', 'Notification', 'PostToolUse', 'Stop', 'StopFailure'].map((eventName) => [
        eventName,
        [{ hooks: [{ type: 'command', command: compatibleHookCommand('workbuddy') }] }],
      ])),
    }, null, 2),
    gemini: JSON.stringify({
      hooks: Object.fromEntries(['BeforeAgent', 'Notification', 'AfterTool', 'AfterAgent'].map((eventName) => [
        eventName,
        [{ hooks: [{ name: 'agent-dock', type: 'command', command: compatibleHookCommand('gemini'), timeout: 3000 }] }],
      ])),
    }, null, 2),
  };
  clipboard.writeText(snippets[source]);
  return {
    ok: true,
    target: source === 'codex'
      ? '~/.codex/hooks.json'
      : source === 'claude'
        ...
        : source === 'workbuddy'
          ...
          : source === 'gemini' ? '~/.gemini/settings.json' : '本机事件接口示例',
  };
});
...
// 缺「屏幕录制」→ CGWindowList 照样返回窗口但标题全空，成果无法匹配原任务窗口；
// 缺「辅助功能」→ 枚举与聚焦窗口无效。
...
async function promptForMissingPermissions() {
  ...
  const { response, checkboxChecked } = await dialog.showMessageBox({
    type: 'info',
    message: `Agent Dock 需要「${names.join('」和「')}」权限`,
    detail: [
      '缺少这些权限时，Agent 成果的窗口跳转会失效。',
      '',
      '授权后需要重新启动 Agent Dock 才会生效。',
  ...
  // 否则用户打开设置面板可能找不到 Agent Dock 这一项、只能手动拖进去。
...
async function rememberPasteTarget() {
  ...
  if (current && !['com.github.Electron', 'com.vibecoding.notch-todo', 'com.dynamicpanel.app', 'com.xiaopu.agentdock'].includes(current.bundleId)) {
...
function ensureFirstRunAutoLaunch() {
  ...
  if (process.platform !== 'darwin' || IS_TEST) return;
...
app.whenReady().then(() => {
  ...
  ensureRecordingsDir();
  applyAppSettings();
  scheduleCompanionPlay(COMPANION_PLAY_FIRST_MS);
  startTaskNotificationServer();
  if (!IS_TEST) void promptForMissingPermissions();

  const pauseCompanion = () => {
    companionSystemPaused = true;
    clearCompanionActivity();
    syncCompanionWindow();
  };
  const resumeCompanion = () => {
    companionSystemPaused = false;
    syncCompanionWindow();
  };
  powerMonitor.on('suspend', pauseCompanion);
  powerMonitor.on('lock-screen', pauseCompanion);
  powerMonitor.on('resume', resumeCompanion);
  powerMonitor.on('unlock-screen', resumeCompanion);
