...
window.NotchTodos = Object.freeze({
  addAgentResult(text, priority) {
    if (!PRIORITIES.includes(priority)) return false;
    return addTodo(priority, text, window.NotchDomain.defaultTodoDeadline(new Date()));
  },
  categories() {
    return PRIORITIES.map((id) => ({ id, name: todoCategoryNames[id] }));
  },
  items() {
    return PRIORITIES.flatMap((priority) => (data[priority] || []).map((item) => ({
      ...item,
      priority,
      category: todoCategoryNames[priority],
    })));
  },
  async open(id, priority) {
    const targetPriority = PRIORITIES.includes(priority)
      ? priority
      : PRIORITIES.find((candidate) => (data[candidate] || []).some((item) => item.id === id));
    if (!targetPriority || !(data[targetPriority] || []).some((item) => item.id === id)) return false;
    await setActiveTab('todo');
    await new Promise((resolve) => requestAnimationFrame(resolve));
    const target = document.querySelector(
      `.todo-item[data-priority="${targetPriority}"][data-id="${CSS.escape(String(id))}"]`
    );
    if (!target) return false;
    const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    target.scrollIntoView({ block: 'center', behavior: reduceMotion ? 'auto' : 'smooth' });
    flashItemClass(targetPriority, id, 'attention-focus');
    target.querySelector('[data-action="edit"]')?.focus({ preventScroll: true });
    return true;
  },
});
window.NotchWorkspaceSnapshot = collectLocalStorageSnapshot;
document.addEventListener('notch:toast', (event) => {
  if (event.detail?.message) showStatusToast(String(event.detail.message));
});
...
function toggleTodo(priority, id) {
  ...
  if (nowDone) {
    ...
    window.notchAPI?.celebrateCompanion?.('todo').catch(() => {});
  }
}
...
function syncPanelAccessibility(expanded) {
  ...
  notch.setAttribute('aria-label', expanded ? '收起 Agent Dock' : '展开 Agent Dock');
...
notch.addEventListener('click', (e) => {
  e.stopPropagation();
  if (!isExpanded) window.notchAPI?.interactCompanion?.().catch(() => {});
  setMode(!isExpanded);
});
...
const ALL_TABS = ['home', 'todo', 'notes', 'recordings', 'clip', 'settings'];
...
pomodoroToggle?.addEventListener('click', () => {
  ...
  window.notchAPI?.setCompanionFocus?.(pomodoroRunning).catch(() => {});
  ...
  if (pomodoroRunning) {
    pomodoroTimer = setInterval(() => {
      ...
      if (pomodoroRemaining <= 0) {
        ...
        pomodoroStarted = false;
        window.notchAPI?.setCompanionFocus?.(false).catch(() => {});
        clearInterval(pomodoroTimer);
        pomodoroTimer = null;
...
const noteContextSelect = document.getElementById('note-context');
...
let noteContexts = new Map();

function noteContextKey(context) {
  if (context?.kind === 'run') return `run:${context.source}:${context.runId}`;
  if (context?.kind === 'project' && context.project) return `project:${context.project}`;
  return '';
}

function noteContextLabel(context) {
  if (context?.kind === 'project') return `项目 · ${context.project}`;
  if (context?.kind === 'run') {
    return `${NOTE_SOURCE_LABELS[context.source] || context.source}${context.project ? ` · ${context.project}` : ''}`;
  }
  return '';
}

function renderNoteContexts(payload = { items: [] }) {
  if (!noteContextSelect) return;
  const current = noteContexts.get(noteContextSelect.value);
  const activeId = localStorage.getItem(NOTE_ACTIVE_ARCHIVE_KEY) || '';
  const archived = loadNoteArchive().find((note) => note.id === activeId)?.context;
  const desiredKey = noteContextKey(current || archived);
  noteContexts = new Map();
  noteContextSelect.replaceChildren(new Option('不关联', ''));
  const runs = Array.isArray(payload.items) ? payload.items : [];
  runs.slice(0, 30).forEach((run) => {
    const context = window.NotchDomain.normalizeNoteContext({
      kind: 'run', source: run.source, runId: run.runId, project: run.project, title: run.title,
    });
    if (!context) return;
    const key = noteContextKey(context);
    noteContexts.set(key, context);
    noteContextSelect.add(new Option(`运行 · ${noteContextLabel(context)} · ${run.title || '未命名'}`, key));
  });
  [...new Set(runs.map((run) => String(run.project || '').trim()).filter(Boolean))].forEach((project) => {
    const context = { kind: 'project', project };
    const key = noteContextKey(context);
    noteContexts.set(key, context);
    noteContextSelect.add(new Option(`项目 · ${project}`, key));
  });
  noteContextSelect.value = noteContexts.has(desiredKey) ? desiredKey : '';
}

async function refreshNoteContexts() {
  if (!window.notchAPI?.listAgentRuns) return;
  const payload = await window.notchAPI.listAgentRuns({ limit: 30 }).catch(() => null);
  renderNoteContexts(payload || { items: [] });
}
...
function renderNotesDetail(notes = loadNoteArchive()) {
  ...
  if (!note) {
    ...
    empty.innerHTML = hasArchive
      ...
      : '<span class="notes-empty-mark" aria-hidden="true">✎</span><strong>还没有保存的笔记</strong><p>在首页的「会话便笺」中写下内容，点击保存后会出现在这里。</p>';
  ...
  const contextLabel = noteContextLabel(note.context);
  time.textContent = `${contextLabel ? `${contextLabel} · ` : ''}更新于 ${noteArchiveTime(note.updatedAt)}`;
...
function updateSavedNotePresentation(note) {
  ...
  if (detailTime) {
    const contextLabel = noteContextLabel(note.context);
    detailTime.textContent = `${contextLabel ? `${contextLabel} · ` : ''}已保存 · ${noteArchiveTime(note.updatedAt)}`;
  }
...
function renderNotesLibrary() {
  ...
  notes.forEach((note) => {
    ...
    const contextLabel = noteContextLabel(note.context);
    excerpt.textContent = contextLabel ? `${contextLabel} · ${noteArchiveExcerpt(note)}` : noteArchiveExcerpt(note);
    const time = document.createElement('time');
...
noteSaveButton?.addEventListener('click', () => {
  ...
  const context = noteContexts.get(noteContextSelect?.value || '') || null;
  ...
  if (existing) {
    ...
    else delete existing.context;
  } else {
    activeId = generateId();
    notes.unshift({ id: activeId, content, createdAt: Date.now(), updatedAt: Date.now(), ...(context ? { context } : {}) });
  }
...
document.addEventListener('notch:notes-changed', renderNotesLibrary);
window.notchAPI?.onAgentRunsChanged?.(renderNoteContexts);
refreshNoteContexts();
...
const HOME_MODULE_REGISTRY = ['agent-status', 'pomodoro', 'recorder', 'attention-center', 'result-inbox', 'note'];
const HOME_ORDER_DEFAULTS = ['agent-status', 'pomodoro', 'attention-center', 'recorder', 'result-inbox', 'note'];
const HOME_SIZE_DEFAULTS = {
  'agent-status': 'small',
  'attention-center': 'large',
  recorder: 'medium',
  'result-inbox': 'medium',
  note: 'medium',
  pomodoro: 'small',
};
const HOME_GRID_MINIMUMS = {
  'agent-status': '2x2',
  pomodoro: '2x1',
  recorder: '4x2',
  'attention-center': '3x2',
  'result-inbox': '3x2',
  note: '3x2',
};
...
const homeTiles = homeBento
  ...
  : [];
const workbenchCat = document.getElementById('workbench-cat');
const workbenchCatMotion = window.matchMedia('(prefers-reduced-motion: reduce)');
let workbenchCatGeneration = 0;
let workbenchCatAnimation = null;
let workbenchCatTimer = null;
let workbenchCatRouteIndex = 0;
let workbenchCatResizeTimer = null;

function workbenchCatCanRun() {
  return Boolean(
    workbenchCat
    && isExpanded
    && activeTab === 'home'
...
function currentWorkbenchCatEdges() {
  const host = document.getElementById('tab-home');
  if (!host) return [];
  const hostRect = host.getBoundingClientRect();
  const edges = homeTiles
    .filter((tile) => !tile.hidden)
    .map((tile) => {
      const rect = tile.getBoundingClientRect();
      return {
        moduleId: tile.dataset.homeModule,
  ...
  const rows = [];
  edges.forEach((edge) => {
    let row = rows.find((candidate) => Math.abs(candidate.top - edge.top) < 2);
    if (!row) {
      row = {
        top: edge.top,
        left: edge.left,
        right: edge.right,
        leftModuleId: edge.moduleId,
        rightModuleId: edge.moduleId,
      };
      rows.push(row);
      return;
    }
    if (edge.left < row.left) {
      row.left = edge.left;
      row.leftModuleId = edge.moduleId;
    }
    if (edge.right > row.right) {
      row.right = edge.right;
      row.rightModuleId = edge.moduleId;
    }
  });
  return rows.length <= 1 ? rows : [rows[0], rows[rows.length - 1]];
}

function stopWorkbenchCatRoute() {
  workbenchCatGeneration += 1;
  if (workbenchCatAnimation) workbenchCatAnimation.cancel();
  if (workbenchCatTimer) clearTimeout(workbenchCatTimer);
  workbenchCatAnimation = null;
  workbenchCatTimer = null;
  if (!workbenchCat) return;
  workbenchCat.hidden = true;
  workbenchCat.dataset.moving = 'false';
  delete workbenchCat.dataset.edgeModule;
  delete workbenchCat.dataset.routeStartModule;
  delete workbenchCat.dataset.routeEndModule;
  workbenchCat.style.removeProperty('opacity');
  workbenchCat.style.removeProperty('transform');
}

function runWorkbenchCatRoute(generation) {
  if (generation !== workbenchCatGeneration || !workbenchCatCanRun()) return;
  const edges = currentWorkbenchCatEdges();
  if (!edges.length) return;
  const edgeNumber = workbenchCatRouteIndex;
  const edge = edges[edgeNumber % edges.length];
  workbenchCatRouteIndex += 1;
  const catWidth = workbenchCat.offsetWidth || 74;
  const catHeight = workbenchCat.offsetHeight || 48;
  const left = edge.left + 16;
  const right = Math.max(left, edge.right - catWidth - 16);
  const y = edge.top - catHeight;
  const reverse = edgeNumber % 2 === 1;
  const start = reverse ? right : left;
  const end = reverse ? left : right;
  const startTransform = `translate3d(${start}px, ${y}px, 0)`;
  const endTransform = `translate3d(${end}px, ${y}px, 0)`;
  workbenchCat.hidden = false;
  workbenchCat.dataset.edgeModule = reverse ? edge.rightModuleId : edge.leftModuleId;
  workbenchCat.dataset.routeStartModule = reverse ? edge.rightModuleId : edge.leftModuleId;
  workbenchCat.dataset.routeEndModule = reverse ? edge.leftModuleId : edge.rightModuleId;
  workbenchCat.dataset.direction = reverse ? 'left' : 'right';

  if (workbenchCatMotion.matches) {
    workbenchCat.dataset.moving = 'false';
    workbenchCat.style.opacity = '1';
    workbenchCat.style.transform = startTransform;
    return;
  }

  workbenchCat.dataset.moving = 'true';
  const distance = Math.abs(end - start);
  workbenchCatAnimation = workbenchCat.animate([
    { opacity: 0, transform: startTransform, offset: 0 },
    { opacity: 1, transform: startTransform, offset: .08 },
    { opacity: 1, transform: endTransform, offset: .92 },
    { opacity: 0, transform: endTransform, offset: 1 },
  ], {
    duration: Math.max(6400, Math.min(18000, distance * 48)),
    easing: 'linear',
    fill: 'forwards',
  });
  workbenchCatAnimation.finished.then(() => {
    if (generation !== workbenchCatGeneration) return;
    workbenchCatAnimation = null;
    workbenchCatTimer = setTimeout(() => runWorkbenchCatRoute(generation), 180);
  }).catch(() => {});
}

function syncWorkbenchCatRoute() {
  stopWorkbenchCatRoute();
  if (!workbenchCatCanRun()) return;
  workbenchCatRouteIndex = 0;
  const generation = workbenchCatGeneration;
  requestAnimationFrame(() => requestAnimationFrame(() => runWorkbenchCatRoute(generation)));
}

document.addEventListener('notch:modechange', syncWorkbenchCatRoute);
document.addEventListener('notch:tabchange', syncWorkbenchCatRoute);
document.addEventListener('notch:home-modules-changed', syncWorkbenchCatRoute);
workbenchCatMotion.addEventListener?.('change', syncWorkbenchCatRoute);
window.addEventListener('resize', () => {
  if (workbenchCatResizeTimer) clearTimeout(workbenchCatResizeTimer);
  workbenchCatResizeTimer = setTimeout(syncWorkbenchCatRoute, 80);
});
...
function loadHomeOrder() {
  try {
    ...
    const saved = Array.isArray(rawSaved)
      ? rawSaved.map((id) => id === 'music' || id === 'character' ? 'agent-status' : id === 'mirror' ? 'result-inbox' : id === 'windows' ? 'attention-center' : id)
      : rawSaved;
    if (
      Array.isArray(saved)
    ...
    if (legacy && typeof legacy === 'object') {
      const migrated = Object.entries(legacy)
        ...
        .map(([id]) => id === 'clock' || id === 'character' || id === 'music' ? 'agent-status' : id === 'mirror' ? 'result-inbox' : id === 'windows' ? 'attention-center' : id)
...
function loadHomeSizes() {
  try {
    const saved = JSON.parse(localStorage.getItem(HOME_SIZES_KEY) || 'null');
    if (saved && typeof saved === 'object' && !Array.isArray(saved)) {
      if (saved.windows && !saved['attention-center']) saved['attention-center'] = saved.windows;
...
function loadHiddenHomeModules() {
  try {
    ...
    const migrated = Array.isArray(parsed)
      ? parsed.map((id) => id === 'windows' ? 'attention-center' : id)
      : parsed;
    const hiddenIds = window.NotchDomain.normalizeHiddenHomeModules(migrated, HOME_MODULE_REGISTRY);
    return {
      hiddenIds,
...
const HOME_LAYOUT_MOTION_MS = 220;
const HOME_LAYOUT_MOTION_EASING = 'cubic-bezier(0.77, 0, 0.175, 1)';
...
function homeGridSizeLabel(value) {
  const dimension = window.NotchDomain.homeWidgetDimensions(value, 12, 4);
  return dimension ? `${dimension.width} × ${dimension.height} 格` : '自适应';
}

function widgetSizeForVariant(size, variant) {
  if (HOME_SIZE_LABELS[size]) return size;
  return variant === 'mini' ? 'mini' : variant === 'compact' ? 'small' : variant === 'wide' ? 'medium' : 'large';
}
...
function applyHomeLayout(layout, { reason = 'initial' } = {}) {
  ...
  homeTiles.forEach((tile) => {
    ...
    if (placement) {
      ...
      tile.dataset.widgetSize = widgetSizeForVariant(size, layout.variants[moduleId]);
    ...
    if (sizeButton) {
      ...
      sizeButton.setAttribute('aria-label', `${homeGridSizeLabel(size)}组件，点击切换预设尺寸；按 Option 加方向键移动模块`);
      sizeButton.setAttribute('aria-keyshortcuts', 'Alt+ArrowLeft Alt+ArrowRight Alt+ArrowUp Alt+ArrowDown');
      sizeButton.title = `组件尺寸：${homeGridSizeLabel(size)}`;
    ...
    const resizeHandle = tile.querySelector('[data-widget-grid-resize]');
    if (resizeHandle) {
      const label = placement ? `${placement.width} × ${placement.height} 格` : homeGridSizeLabel(size);
      resizeHandle.setAttribute('aria-label', `调整组件尺寸，当前 ${label}；使用方向键按格调整`);
      resizeHandle.setAttribute('aria-keyshortcuts', 'ArrowLeft ArrowRight ArrowUp ArrowDown');
      resizeHandle.title = `拖动调整尺寸 · ${label}`;
      resizeHandle.hidden = automaticLayout || homeLayoutReadOnly;
      resizeHandle.disabled = automaticLayout || homeLayoutReadOnly;
      resizeHandle.tabIndex = automaticLayout || homeLayoutReadOnly ? -1 : 0;
    }
  });
  animateCommittedHomeLayout(reason, beforeState);
  requestAnimationFrame(syncWorkbenchCatRoute);
}
...
homeTiles.forEach((tile) => {
  ...
  resizeHandle.className = 'widget-resize-handle motion-icon';
  resizeHandle.dataset.widgetGridResize = tile.dataset.homeModule;
  resizeHandle.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" aria-hidden="true"><path d="M10 20 20 10M15 20l5-5M5 20 20 5"/></svg>';
...
if (homeBento) {
  ...
  resizePreview.className = 'home-resize-preview';
  resizePreview.setAttribute('aria-hidden', 'true');
  resizePreview.innerHTML = '<span></span>';
  homeBento.appendChild(resizePreview);

  const clearDropTarget = () => {
    homeTiles.filter((tile) => !tile.hidden).forEach((tile) => tile.classList.remove('layout-drop-target'));
  };

  const moveHomeModuleByKeyboard = (tile, offset) => {
    const visibleIds = homeOrder.filter((id) => !hiddenHomeModules.includes(id));
    const visibleIndex = visibleIds.indexOf(tile.dataset.homeModule);
    const targetId = visibleIds[visibleIndex + offset];
    if (!targetId) return false;
    const sourceIndex = homeOrder.indexOf(tile.dataset.homeModule);
    ...
    requestAnimationFrame(() => tile.querySelector('[data-widget-size-cycle]')?.focus({ preventScroll: true }));
    return true;
  };

  const commitHomeGridResize = (tile, requested, focusHandle = false) => {
    const result = window.NotchDomain.resizeHomeWidgetGrid(
      homeOrder,
      homeSizes,
      hiddenHomeModules,
      tile.dataset.homeModule,
      requested,
      HOME_GRID_MINIMUMS,
      12,
      4
    ...
    if (!result) {
      showStatusToast('当前尺寸无法放入网格');
      return false;
    }
    homeSizes = result.sizes;
    applyHomeLayout(result.layout, { reason: 'size' });
    saveHomeLayout();
    const snapped = result.applied.width !== result.requested.width || result.applied.height !== result.requested.height;
    showStatusToast(`${result.applied.width} × ${result.applied.height} 格${snapped ? ' · 已吸附到最近可用尺寸' : ' · 其他模块已自适应'}`);
    if (focusHandle) requestAnimationFrame(() => tile.querySelector('[data-widget-grid-resize]')?.focus({ preventScroll: true }));
    return true;
  };

  const updateHomeResizePreview = (event) => {
    if (!resizeState || resizeState.pointerId !== event.pointerId) return;
    const surface = homeBento.getBoundingClientRect();
    const style = getComputedStyle(homeBento);
    const gapX = Number.parseFloat(style.columnGap) || 0;
    const gapY = Number.parseFloat(style.rowGap) || 0;
    const cellWidth = (surface.width - gapX * 11) / 12;
    const cellHeight = (surface.height - gapY * 3) / 4;
    const minimum = window.NotchDomain.homeWidgetDimensions(
      HOME_GRID_MINIMUMS[resizeState.tile.dataset.homeModule],
      12,
      4
    );
    const width = Math.max(minimum.width, Math.min(12,
      resizeState.startWidth + Math.round((event.clientX - resizeState.startX) / (cellWidth + gapX))));
    const height = Math.max(minimum.height, Math.min(4,
      resizeState.startHeight + Math.round((event.clientY - resizeState.startY) / (cellHeight + gapY))));
    const widthPx = width * cellWidth + (width - 1) * gapX;
    const heightPx = height * cellHeight + (height - 1) * gapY;
    resizeState.requested = { width, height };
    resizePreview.style.left = `${Math.max(0, Math.min(resizeState.startLeft, surface.width - widthPx))}px`;
    resizePreview.style.top = `${Math.max(0, Math.min(resizeState.startTop, surface.height - heightPx))}px`;
    resizePreview.style.width = `${widthPx}px`;
    resizePreview.style.height = `${heightPx}px`;
    resizePreview.querySelector('span').textContent = `${width} × ${height}`;
  };

  const finishHomeResize = (event, cancelled = false) => {
    if (!resizeState || (event?.pointerId != null && resizeState.pointerId !== event.pointerId)) return;
    const { tile, handle, pointerId, requested } = resizeState;
    if (handle.hasPointerCapture?.(pointerId)) handle.releasePointerCapture(pointerId);
    tile.classList.remove('is-resizing');
    homeBento.classList.remove('layout-resizing');
    resizePreview.classList.remove('is-visible');
    resizeState = null;
    if (!cancelled) commitHomeGridResize(tile, requested);
    suppressHomeClickUntil = Date.now() + 180;
  };
  ...
  homeBento.addEventListener('pointerdown', (event) => {
    const handle = event.target.closest('[data-widget-grid-resize]');
    if (handle && event.button === 0 && event.isPrimary !== false) {
      const tile = handle.closest('[data-home-module]');
      if (!tile || tile.hidden || handle.disabled) return;
      event.preventDefault();
      event.stopPropagation();
      const surface = homeBento.getBoundingClientRect();
      const rect = tile.getBoundingClientRect();
      ...
      resizeState = {
        ...
        startWidth: Number(tile.dataset.layoutWidth) || 2,
        startHeight: Number(tile.dataset.layoutHeight) || 1,
        startLeft: rect.left - surface.left,
        startTop: rect.top - surface.top,
        requested: {
          width: Number(tile.dataset.layoutWidth) || 2,
          height: Number(tile.dataset.layoutHeight) || 1,
        },
      };
      tile.classList.add('is-resizing');
      homeBento.classList.add('layout-resizing');
      resizePreview.classList.add('is-visible');
      updateHomeResizePreview(event);
      return;
    }
  ...
  homeBento.addEventListener('keydown', (event) => {
    if (event.key === 'Escape' && resizeState) {
      event.preventDefault();
      finishHomeResize(null, true);
      return;
    }
    const resizeHandle = event.target.closest('[data-widget-grid-resize]');
    if (resizeHandle && ['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown'].includes(event.key)) {
      const tile = resizeHandle.closest('[data-home-module]');
      const current = {
        width: Number(tile.dataset.layoutWidth) || 2,
        height: Number(tile.dataset.layoutHeight) || 1,
      };
      const minimum = window.NotchDomain.homeWidgetDimensions(HOME_GRID_MINIMUMS[tile.dataset.homeModule], 12, 4);
      const requested = {
        width: Math.max(minimum.width, Math.min(12,
          current.width + (event.key === 'ArrowRight' ? 1 : event.key === 'ArrowLeft' ? -1 : 0))),
        height: Math.max(minimum.height, Math.min(4,
          current.height + (event.key === 'ArrowDown' ? 1 : event.key === 'ArrowUp' ? -1 : 0))),
      };
      event.preventDefault();
      event.stopPropagation();
      commitHomeGridResize(tile, requested, true);
      return;
    }
    ...
    event.preventDefault();
    event.stopPropagation();
    moveHomeModuleByKeyboard(tile, ['ArrowLeft', 'ArrowUp'].includes(event.key) ? -1 : 1);
  });
  ...
  homeBento.addEventListener('click', (event) => {
    ...
    const currentDimension = window.NotchDomain.homeWidgetDimensions(current, 12, 4);
    const currentIndex = sequence.indexOf(current);
    const nearestIndex = currentIndex >= 0 ? currentIndex : sequence
      .map((size, index) => ({ index, dimension: window.NotchDomain.homeWidgetDimensions(size, 12, 4) }))
      .sort((left, right) => (
        Math.abs(left.dimension.width - currentDimension.width) + Math.abs(left.dimension.height - currentDimension.height)
        - Math.abs(right.dimension.width - currentDimension.width) - Math.abs(right.dimension.height - currentDimension.height)
      ))[0].index;
    commitHomeGridResize(sizeButton.closest('[data-home-module]'), sequence[(nearestIndex + 1) % sequence.length], true);
  });
  ...
  homeBento.addEventListener('pointerup', (event) => resizeState ? finishHomeResize(event) : finishHomeDrag(event));
  homeBento.addEventListener('pointercancel', (event) => resizeState ? finishHomeResize(event, true) : finishHomeDrag(event, true));
